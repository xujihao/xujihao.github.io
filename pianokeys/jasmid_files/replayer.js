	...
	function handleEvent() {
		...
		switch (event.type) {
			...
			case 'channel':
				switch (event.subtype) {
					case 'noteOn':
						pianokey=$(keyborads[event.noteNumber-21]);
						channels[event.channel].noteOn(event.noteNumber, event.velocity);
						if(pianokey.hasClass('key-black')){
							pianokey.css('backgroundColor','#f0f');
							pianokey.css('transform','rotateY(20deg)');
						}else{
							pianokey.css('backgroundColor','#ff0');
							pianokey.find('span').css('fontSize',pianokey.width()/2+1);
							pianokey.css('transform','rotateY(15deg)');
						}
						break;
					case 'noteOff':
						pianokey=$(keyborads[event.noteNumber-21]);
						...
						if(pianokey.hasClass('key-black')){
							pianokey.css('backgroundColor','#000');
						}
						else{
							//pianokey.css('backgroundColor','#fff');
							setTimeout(function(){
								pianokey.css('backgroundColor','#fff');
							},50);
							pianokey.find('span').css('fontSize',pianokey.width()/2);
						}
						pianokey.css('transform','rotateY(0deg)');
						break;
					case 'programChange':
